...
int NVMJournal::replay_journal()
{
    ...
    do {
        ...
        ret = check_crc32((uint32_t*)&header, sizeof(header)/sizeof(uint32_t));
        if (ret) {
...
NVMJournal::NVMJournal(string dev, string c, BackStore *s, Finisher *fin)
    :RWJournal (s, fin),
    cur_seq(1),
    mode(AI_MODE/*WRITE_THROUGH_MODE*/),
    write_pos(0),
    meta_sync_pos(0),
    data_sync_pos(0),
    ...
    buffer_cache(256*1024, 768*1024*1024),
    cache_lock("NVMJournal::cache_lock", false, true, false, g_ceph_context),
    enable_cache(false)
{
}
...
int NVMJournal::read_entry(uint64_t &pos, uint64_t &next_seq, Op *op)
{
    ...
    ssize_t size = h.length - h.data_len;
    if (!size)
        return -1;
...
void NVMJournal::do_aio_write(bufferlist &entry, uint64_t seq)
{
    ...
    while (entry.length() > 0) {
        ...
        aio.iov = iov;
        aio.next_write_pos = write_pos + aio.len;
    ...
    uint64_t meta_not_sync_pos = 0;
    {
        ...
        while (it != aio_queue.end() && it->done) {
            ...
            if (it->seq) {
                ...
                meta_not_sync_pos = it->next_write_pos;
            }
            aio_num --;
            ++it;
        }
        ...
        bufferlist& bl, uint64_t entry_pos, uint32_t boff)
{
...

    {
        ...
        if (enable_cache && !replay) { /* cache */
            Mutex::Locker lock(cache_lock);
            buffer_cache.add(bh, bl);
        }
    }
...
void NVMJournal::merge_new_bh(ObjectRef obj, BufferHead* new_bh)
{
        ...
        if (new_start <= start) {
            ...
            /* new_start, start, new_end, end */
                ...
                if (enable_cache) { /* cache */
                    Mutex::Locker lock(cache_lock);
                    bufferlist bl;
                    bool found = buffer_cache.lookup(bh, bl);
                    if (found) {
                        bufferlist newbl;
                        newbl.substr_of(bl, new_end-start, end-new_end);
                        buffer_cache.purge(bh);
                        buffer_cache.add(bh, newbl);
                    }
                }
            ...
            /*start, new_start, new_end, end */
                ...
                if (enable_cache) { /*cache*/
                    ...
                    if (found) {
                        ...
                        buffer_cache.purge(bh);
                        buffer_cache.add(right, right_bl);
                        buffer_cache.add(left, left_bl);
                    }
                }
                if (!bh->owner) 
                    delete bh;
    ...
    bool rand_mark_hot = !obj->s_write && rand()%4==3;
    ssize_t rand_addition = 0.34*len;
    ...
    /*when write the object the first time, mark it hot with prob = 1/4*/
    if (rand_mark_hot) {
        obj->s_overlap += rand_addition;
        obj->s_ops += 2;
    }
...
void NVMJournal::map_read(ObjectRef obj, uint32_t off, uint32_t end,
        ...
        bool trunc_as_zero)
{
    dout(8) << "object = " << obj << ", off = " << off << ", end = " << end << dendl;
            ...
            else if (pbh->ext.end < end) 
            {
                ...
                if (pbh->bentry != BufferHead::ZERO) {
                    ...
                    if (enable_cache) {
                        Mutex::Locker lock(cache_lock);
                        bufferlist bl;
                        bool found = buffer_cache.lookup(pbh, bl);
                        if (found) {
                            bufferlist hitbl;
...
int NVMJournal::do_read(ReadOp &op)
{
    ...
    for (map<uint32_t, uint32_t>::iterator p = op.hits.begin();
        ...
        if (bentry != BufferHead::ZERO && bentry != BufferHead::TRUNC) {
            ...
            uint32_t r = safe_pread64(fd, ptr.c_str(), ptr.length(), start);
            if(r != ptr.length()) {
                dout(5) << "safe_pread64: fd = " << fd2
                    << ", length = " << ptr.length()
                    << ", start = " << start << dendl;
                assert(0);
            } 
...
bool NVMJournal::LRUCache::lookup(BufferHead* key, bufferlist &value)
{
